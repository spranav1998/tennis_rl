...
class MADDPG:
    ...
    def save(self, actor_filename, critic_filename):
        for i, agent in enumerate(self.agents):
            torch.save(agent.actor_local.state_dict(), f'{actor_filename}.pth')
            torch.save(agent.critic_local.state_dict(), f'{critic_filename}.pth')
    ...
    def load(self, actor_filename, critic_filename):
        for i, agent in enumerate(self.agents):
            agent.actor_local.load_state_dict(torch.load(f'{actor_filename}.pth'))
            agent.critic_local.load_state_dict(torch.load(f'{critic_filename}.pth'))
